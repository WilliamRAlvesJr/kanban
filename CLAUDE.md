...
A capability `board-management` está implementada. O pacote `com.william.kanban.board` tem `Board`, `BoardRepository`, `BoardService`, `BoardController`, `BoardNotFoundException`, `BoardLimits` e os records `CreateBoardRequest`, `UpdateBoardRequest` e `BoardResponse`. `POST /boards` responde 201 com o dono tirado do token; `GET /boards` lista os quadros da conta do mais recente para o mais antigo, com o filtro opcional `archived`; `GET /boards/{id}` responde 200; `PUT /boards/{id}` substitui `name` e `description`; `POST /boards/{id}/archive` e `POST /boards/{id}/restore` carimbam e limpam `archived_at`, e a operação repetida não altera o quadro. Entrada inválida responde 400. Toda leitura por id passa por `findByIdAndOwnerId`, então quadro de outra conta responde 404 com o mesmo corpo de quadro inexistente. `created_at`, `updated_at` e `archived_at` vêm do relógio da JVM, e as colunas de tempo de `boards` não têm `DEFAULT`.

O `GlobalExceptionHandler`, em `com.william.kanban.shared`, converte essas exceções em `ProblemDetail`, e o `OpenApiResponsesConfig`, no mesmo pacote, documenta essas respostas no OpenAPI. Coluna e card estão por fazer.
